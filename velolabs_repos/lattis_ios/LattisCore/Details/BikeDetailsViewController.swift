...
public class BikeDetailsViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        bikeImageView.kf.setImage(with: bike.picture, completionHandler:  { [weak self] result in
            self?.renderHeader()
        })
        ...
        if let surcharge = bike.surchargePrice {
            ...
            if let desc = bike.surchargeDescription {
                priceSection.append(.info(desc))
            }
        }
        if bike.isFree == false, let parking = bike.parkingPrice {
            priceSection += [
        ...
        var network: [BikeInfo] = [.fleet(bike.fleetName)]
        if bike.terms != nil {
        ...
        sections = [
            .init(title: nil, items: [.bike(bike)]),
            ...
            .init(title: "bike_detail_label_about_the_bike".localized(), items: [
                .tuple("bike_detail_label_name".localized(), bike.name),
                .tuple("bike_detail_label_model".localized(), bike.kind.rawValue.capitalized),
        ...
        tableView.register(BikeInfoBikeCell.self, forCellReuseIdentifier: "bike")
    ...
    fileprivate func openParkingZones() {
        let zones = ParkingZonesViewController(bike.fleetId)
        let map = AppRouter.shared.map(zones)
        present(map, animated: true, completion: nil)
    }
    
    fileprivate func openTermsAndConditions() {
        guard let terms = bike.terms, let url = URL(string: terms) else { return warning() }
        let safari = SFSafariViewController(url: url)
        present(safari, animated: true, completion: nil)
    }
}

extension BikeDetailsViewController: UITableViewDataSource, UITableViewDelegate {
