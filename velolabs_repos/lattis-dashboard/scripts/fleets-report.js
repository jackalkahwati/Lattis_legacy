...
//       .select('bikes.bike_id', 'fleet_id', 'bikes_audit_log.*')
...
const output = createWriteStream(`${process.cwd()}/files/CSV/fleetReport.csv`, {
  encoding: 'utf8'
})
...
const formatMassiveData = (history) => {
  ...
  bikesHistoryObject.change_before = {}
    ...
    for (let h of history) {
      if (mAY === h.time) {
        bikesHistoryObject[mAY] = bikesHistoryObject[mAY]
          ...
          : [h.change_after]
        bikesHistoryObject['change_before'][mAY] = bikesHistoryObject['change_before'][mAY]
          ? [...bikesHistoryObject['change_before'][mAY], h.change_bofore]
          : [h.change_before]
...
const getFleetBikeHistory = async () => {
  for (let bikeId of bikeIds) {
    ...
    console.log(
      '🚀 ~ file: fleets-report.js ~ getFleetBikeHistory ~ history',
      history
    )
...
genReport()
  .then(() => {
    getFleetBikeHistory()
      ...
      .catch((err) => {
        console.log(
          '🚀 ~ file: fleets-report.js ~ getFleetBikeHistory ~ err',
          err
        )
      })
  })
