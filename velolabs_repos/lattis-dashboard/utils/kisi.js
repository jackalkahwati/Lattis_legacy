...
class Kisi {
  async _init (fleetId) {
    try {
      ...
      const integration = await db
        ...
        .first()
      if (!integration) {
        throw new Error(`Kisi integration not found for the fleet. Please add integration`)
      }
  ...
  async fetchLock (fleetId, lockId) {
    try {
      ...
      const message = error.response && error.response.data && error.response.data.message
      throw new Error(
        error.message || message || `Error fetching Kisi lock for fleet ${this.fleetId}: `
      )
    }
  }
  ...
  async lock (fleetId, lockId) {
    try {
      ...
      const message = error.response && error.response.data && error.response.data.message
      throw new Error(
        message || `Error locking Kisi lock for fleet ${this.fleetId}: `
      )
    }
  }
  ...
  async unlock (fleetId, lockId) {
    try {
      ...
      const message = error.response && error.response.data && error.response.data.message
      throw new Error(
        message || `Error unlocking Kisi locks for fleet ${this.fleetId}: `
      )
    }
  }
}
