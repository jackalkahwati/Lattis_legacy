...
static hal_qdec_event_t hal_event;
static void qdec_handler(void* p_event_data, uint16_t event_size)
{
    UNUSED_PARAMETER(p_event_data);
    UNUSED_PARAMETER(event_size);
    s_qdec_int_callback(hal_event);
}
...
void QDEC_IRQHandler(void)
{
    ...
    {
        NRF_QDEC->EVENTS_REPORTRDY = 0;
        hal_event = hal_qdec_event_reportrdy;
    }
    else if (NRF_QDEC->EVENTS_SAMPLERDY)
    {
        NRF_QDEC->EVENTS_SAMPLERDY = 0;
        hal_event = hal_qdec_event_samplerdy;
    }
    else if (NRF_QDEC->EVENTS_ACCOF)
    {
        NRF_QDEC->EVENTS_ACCOF = 0;
        hal_event = hal_qdec_event_accof;
    }

    app_sched_event_put(&hal_event, sizeof(hal_event), qdec_handler);
}
