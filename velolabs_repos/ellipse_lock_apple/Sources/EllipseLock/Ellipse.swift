...
public class Ellipse: NSObject {
    ...
    fileprivate var needReset = false
    ...
    public func connect(handler: EllipseDelegate? = nil, secret: String? = nil) {
        ...
        startTimer()
        guard unlockPin == nil, !needReset else {
    ...
    public var extendedOpenTime: Bool? {
        set {
            guard !capTouchBytes.isEmpty, let time = newValue else { return }
            capTouchBytes[0] = time ? 0xFF : 0x00
            _ = try? write(bytes: capTouchBytes, to: .capTouch, for: .configuration)
        }
        get {
            guard !capTouchBytes.isEmpty else { return nil }
            return capTouchBytes[0] > 0x00
        }
    }
