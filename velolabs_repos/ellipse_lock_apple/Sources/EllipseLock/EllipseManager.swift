...
public class EllipseManager: NSObject {
    ...
    public static let uninitialized = EllipseManager()
    ...
    @discardableResult fileprivate func addLocks(peripherals: [CBPeripheral]) -> [Ellipse] {
        ...
        ellipses.forEach { (ellipse) in
            ...
            ellipse.sign = { [unowned self, unowned ellipse] closure in
                guard let api = self.api else { return ellipse.connection = .failed(EllipseError.noAPIfound) }
                api.sign(lockWith: ellipse.macId, completion: { (result) in
                    switch result {
