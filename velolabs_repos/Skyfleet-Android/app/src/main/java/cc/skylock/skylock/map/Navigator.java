package cc.skylock.skylock.map;
...
public class Navigator {
	...
	private class PathCreator extends AsyncTask<Void,Void,Directions> {
		...
		protected Directions doInBackground(Void... params) {
			...
			LockWebServiceApi lockWebServiceApi = RetofitRestAdapter.getClient(url).create(LockWebServiceApi.class);
			Call<String> lockRoute = lockWebServiceApi.getRouteOfYouLock();

			lockRoute.enqueue(new Callback<String>() {
				@Override
