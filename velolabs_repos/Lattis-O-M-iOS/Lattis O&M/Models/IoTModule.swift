...
extension IoTModule {
    init(axa: AxaBLE.Lock, fleet: Int) {
        controllerId = nil
        bikeId = nil
        fleetId = fleet
        key = axa.id
        vendor = .AXA
        deviceType = "lock"
        fwVersion = axa.fwVersion
        hwVersion = axa.hwVersion
        make = "Axa eRL"
        model = axa.modelNumber
        status = "active"
        batteryLevel = axa.batteryLevel
    }
}
