...
import static com.lattis.lattis.presentation.library.sliding.SlideUp.State.HIDDEN;
import static com.lattis.lattis.presentation.library.sliding.SlideUp.State.SHOWED;
...
public class SlideUp implements View.OnTouchListener, ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener, LoggerNotifier {
    ...
    private void init() {
        ...
        if(mBuilder.mAlsoScrollView != null && mBuilder.isTouchEnabled) {
...
//        float percents = (visibleDistance) * 100 / mViewHeight;
        float percents = 100*(mBuilder.mSliderView.getY()/(mBuilder.mAlsoScrollView.getY()-50));
        percents = percents>98?100:percents;
        notifyPercentChanged(percents);
    }
