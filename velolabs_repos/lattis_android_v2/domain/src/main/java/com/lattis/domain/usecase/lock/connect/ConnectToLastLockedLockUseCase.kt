...
class ConnectToLastLockedLockUseCase @Inject internal constructor(
    threadExecutor: ThreadExecutor,
    postExecutionThread: PostExecutionThread,
    bluetoothRepository: BluetoothRepository,
    lockRepository: LockRepository,
    saSOrPSLockRepository: SaSOrPSLockRepository
) : ConnectToLockUseCase(threadExecutor, postExecutionThread, bluetoothRepository, lockRepository,saSOrPSLockRepository) {

    override fun buildUseCaseObservable(): Observable<Lock.Connection.Status> {
        return connectToLastConnectedLock(lockVendor)
    }
}