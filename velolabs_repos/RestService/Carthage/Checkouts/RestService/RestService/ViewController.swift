...
//        Oval.users.refreshTokens(success: {}, fail: {print($0)})
    ...
    @IBAction func signUp(_ sender: Any) {
        Oval.users.registration(user: regUser, success: { (result) in
            print(result)
        }) { (error) in
    ...
    @IBAction func confirm(_ sender: Any) {
        Oval.users.confirm(signIn: codeField.text!, success: { (result) in
    ...
    @IBAction func confirmForgot(_ sender: Any) {
        Oval.users.confirm(forgot: forgotCodeField.text!, phone: "+79600448886", password: "vefq2nfq", success: {_ in}, fail: {_ in})
    }
