...
extension Accelerometer.Coordinate {
    init(bytes: [UInt8], x: Int, y: Int, z: Int, count: Int) {
        self.x = EllipseBLE.convert(bytes: Array(bytes[x..<x+count]))
        self.y = EllipseBLE.convert(bytes: Array(bytes[y..<y+count]))
        self.z = EllipseBLE.convert(bytes: Array(bytes[z..<z+count]))
    }
}
