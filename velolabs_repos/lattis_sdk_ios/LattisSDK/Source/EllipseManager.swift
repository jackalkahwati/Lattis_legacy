...
public class EllipseManager: NSObject {
    @objc public static let shared = EllipseManager()
    ...
    fileprivate let queue = DispatchQueue(label: "BLE.EllipseManager.Queue", qos: .utility)
...
//        invalidateTimer()
    ...
    fileprivate func startTimer() {
        invalidateTimer()
        locksTimer = DispatchSource.makeTimerSource()
    ...
    @discardableResult fileprivate func addLocks(peripherals: [CBPeripheral]) -> [Ellipse] {
        ...
        ellipses.forEach { (ellipse) in
            ...
            ellipse.sign = { [unowned self, unowned ellipse] closure in
                self.network.sign(lockWith: ellipse.macId, completion: { (result) in
                    switch result {
