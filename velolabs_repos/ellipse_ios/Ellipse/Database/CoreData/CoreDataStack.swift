...
final class CoreDataStack {
    ...
    func setup(_ name: String = "Ellipse", userId: Int, completion: @escaping () -> ()) {
        self.userId = userId
    ...
    private func save() throws {
        ...
        let handlers = Array(self.handlers.allObjects.filter { (handler) -> Bool in
            return self.mainContext.deletedObjects.contains(where: {contains($0, handler)}) ||
                self.mainContext.insertedObjects.contains(where: {contains($0, handler)}) ||
                self.mainContext.updatedObjects.contains(where: {contains($0, handler)})
        })
        try mainContext.save()
        handlers.forEach{$0.callback()}
    }
}
